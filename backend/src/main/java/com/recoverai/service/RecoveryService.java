...
public class RecoveryService {
    ...
    @Transactional public synchronized ExecutionResult execute(String id) {
        ...
        if (action != RecoveryAction.CREATE_PAYMENT_LINK) {
            ...
            if (next == RecoveryStatus.ESCALATED) {
                ...
                log(id, "SYSTEM", "RECOVERY_ACTION_RECORDED", "Approved recovery action recorded", Map.of("action", action));
            }
    ...
    @Transactional public int stopOpenCases() {
        List<RecoveryCase> openCases = cases.findAll().stream().filter(recoveryCase -> recoveryCase.status() != RecoveryStatus.RECOVERED && recoveryCase.status() != RecoveryStatus.STOPPED && recoveryCase.status() != RecoveryStatus.ESCALATED).toList();
