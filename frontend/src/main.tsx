...
function App() {
  const [view, setView] = useState<'overview' | 'cases' | 'simulation' | 'firestore' | 'policies'>('overview');
  ...
  const [connectionState, setConnectionState] = useState<'checking' | 'live' | 'offline'>('checking');
  ...
  const caseBreakdown = useMemo(() => ({
    ...
    waiting: cases.filter(recoveryCase => recoveryCase.status === 'WAITING_CUSTOMER').length,
    escalated: cases.filter(recoveryCase => recoveryCase.status === 'ESCALATED').length,
    recovered: cases.filter(recoveryCase => recoveryCase.status === 'RECOVERED').length
  }), [cases, openCases]);
  ...
  return <div className="shell">
    <aside>
      ...
      <div className="sidebar-integrations"><p>CONNECTION</p><button className={`integration-button ${view === 'firestore' ? 'active' : ''}`} onClick={() => setView('firestore')}><Database size={15} /> Firestore <b className={`connection-${connectionState}`}>{connectionState === 'live' ? 'Live' : connectionState === 'checking' ? 'Checking' : 'Offline'}</b></button><button className={`integration-button ${view === 'policies' ? 'active' : ''}`} onClick={() => setView('policies')}><ShieldCheck size={15} /> Policy controls</button>{lastSyncedAt && <small className="last-sync">Synced {lastSyncedAt.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</small>}</div>
    ...
    <main>
      ...
      {view === 'simulation' && <article className="card simulation"><section className="simulation-hero"><span className="simulation-icon"><FlaskConical size={19} /></span><div><em>POLICY EVALUATION LAB</em><h2>Synthetic evaluation</h2><p>Run a deterministic 240-case policy benchmark. It never contacts Razorpay or represents real merchant revenue.</p><span className="simulation-note">Safe synthetic data only</span></div><button onClick={simulate} disabled={busy}>Run 240-case benchmark</button></section>{evaluation && <><mark>SIMULATED · {evaluation.datasetSize} cases · seed {evaluation.seed}</mark><section className="metrics"><Metric name="At risk" value={money(evaluation.totalAtRisk)} /><Metric name="Attempted" value={money(evaluation.totalAttempted)} /><Metric name="Recovered" value={money(evaluation.totalRecovered)} good /><Metric name="Recovery rate" value={`${Math.round(evaluation.recoveryRate * 100)}%`} /></section><p><b>{evaluation.policyApproved} approved</b> · {evaluation.policyBlocked} blocked by policy</p><div className="breakdown">{Object.entries(evaluation.byRiskType).map(([type, values]) => <div key={type}><span>{label(type)}</span><i><b style={{ width: `${values.atRisk ? Math.round(values.recovered / values.atRisk * 100) : 0}%` }} /></i><small>{money(values.recovered)} simulated recovered</small></div>)}</div></>}{evaluationHistory.length > 0 && <section className="evaluation-history"><h3>Recent evaluation runs</h3>{evaluationHistory.map(run => { const rate = Math.round(run.result.recoveryRate * 100); return <div key={run.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}><div style={{ display: 'grid', gap: 4 }}><b style={{ fontSize: 13 }}>Policy benchmark</b><small>{new Date(run.createdAt).toLocaleString('en-IN')} · {run.result.datasetSize} synthetic cases</small></div><div style={{ display: 'grid', gridTemplateColumns: 'auto auto', gap: '5px 10px', alignItems: 'center', minWidth: 220 }}><span style={{ fontSize: 11, color: '#6d829b' }}>Recovery rate</span><strong style={{ fontSize: 13, textAlign: 'right' }}>{rate}%</strong><i style={{ gridColumn: '1 / -1', width: '100%' }}><b style={{ width: `${rate}%` }} /></i></div></div>; })}</section>}</article>}

      {view === 'firestore' && <section className="inspector-page"><article className="card page-intro"><span className="page-icon"><Database size={19} /></span><div><em>SAFE BACKEND SNAPSHOT</em><h2>Firestore recovery data</h2><p>This page shows data loaded from Firestore through the secured Spring Boot API. Browser clients never receive database credentials or direct Firestore access.</p></div><span className={`page-status connection-text-${connectionState}`}>{connectionState === 'live' ? 'Synced' : connectionState === 'checking' ? 'Checking connection' : 'Backend offline'}</span></article><section className="snapshot-grid page-grid"><div><small>Total recovery cases</small><b>{cases.length}</b></div><div><small>Open workflows</small><b>{caseBreakdown.open}</b></div><div><small>Waiting for customer</small><b>{caseBreakdown.waiting}</b></div><div><small>Escalated to human</small><b>{caseBreakdown.escalated}</b></div><div><small>Recovered</small><b>{caseBreakdown.recovered}</b></div><div><small>Revenue recovered</small><b>{money(summary?.revenueRecovered ?? 0)}</b></div></section><section className="page-detail-grid"><article className="card"><h3>What is synced</h3><p>Recovery cases, payment-link state, audit events, scheduled tasks, webhook events, and evaluation history are persisted in Firestore.</p><span className="detail-tag">Read through secured API</span></article><article className="card"><h3>Connection model</h3><ol className="compact-list"><li>Dashboard requests the Spring Boot API.</li><li>The backend reads authorised Firestore data.</li><li>Only safe response data returns to this browser.</li></ol></article></section><article className="card data-table"><div><h2>Recovery case data</h2><small>{lastSyncedAt ? `Last successful API sync: ${lastSyncedAt.toLocaleString('en-IN')}` : 'Waiting for a successful backend sync.'}</small></div><Rows cases={cases} select={id => { selectCase(id); setView('cases'); }} /></article></section>}

      {view === 'policies' && <section className="inspector-page"><article className="card page-intro"><span className="page-icon"><ShieldCheck size={19} /></span><div><em>AUTONOMOUS SAFETY BOUNDARIES</em><h2>Policy controls</h2><p>AI may recommend a recovery strategy, but every executable action must pass these backend-enforced policy checks.</p></div><span className="page-status">Backend enforced</span></article><article className="card policy-page"><h2>Recovery action rules</h2><p>These guardrails prevent unsafe recovery actions and make every decision explainable in the audit timeline.</p><ul className="policy-list"><li><Check size={15} /><span><b>Captured payment block</b>Never act after payment has already been captured.</span></li><li><Check size={15} /><span><b>Maximum attempt limit</b>Recovery stops after the configured attempt threshold.</span></li><li><Check size={15} /><span><b>Contact permission</b>Payment links and reminders require approved contact permission.</span></li><li><Check size={15} /><span><b>Duplicate-link prevention</b>Only one equivalent active payment link is permitted.</span></li><li><Check size={15} /><span><b>Human ownership</b>Escalated, stopped, recovered, and unrecoverable cases cannot be automated.</span></li><li><Check size={15} /><span><b>Valid recovery amount</b>Recovery actions require a positive at-risk amount and recognised action type.</span></li></ul></article><section className="page-detail-grid"><article className="card"><h3>Decision sequence</h3><ol className="compact-list"><li>Risk is detected or created by the merchant.</li><li>AI or deterministic rules recommend an intervention.</li><li>Policy evaluates every required safety check.</li><li>Only an approved, bounded action can run.</li></ol></article><article className="card"><h3>When automation stops</h3><p>Recovery pauses on capture, maximum attempts, no contact permission, duplicate links, stopped cases, or human escalation.</p><span className="detail-tag">Human review remains in control</span></article></section><article className="card policy-audit-note"><h3>Audit-first recovery</h3><p>Every analysis, policy approval or block, payment-link creation, task completion, webhook event, recovery result, and human handoff is retained in the case audit timeline.</p></article></section>}
    </main>
  </div>;
}
